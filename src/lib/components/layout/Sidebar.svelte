<script>
	...
	$: isActive = (path) => browser ? $page.url.pathname === path : false;
  </script>
  
  <aside class="fixed inset-y-0 left-0 w-64 bg-slate-800 text-white flex flex-col">
	<!-- Logo/Header -->
	<div class="p-4 border-b border-slate-700">
	  <h1 class="text-xl font-bold">LocationMetric</h1>
	  ...
	  <ul class="space-y-1">
		{#each navItems as item}
		  <li>
			<a 
			  href={item.href}
			  class="flex items-center px-4 py-3 {isActive(item.href) ? 
